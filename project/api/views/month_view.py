...
from api.serializers import MonthSerializer, MonthMinimalSerializer
...
class MonthView(APIView):
    ...
    def get(self, request, id=None):
        try:
            ...
            return Response({'month_detail': MonthSerializer(month).data}, status=status.HTTP_200_OK)
        except Http404:
...
class MonthListView(APIView):
    @extend_schema(
            ...
            responses={
                200: MonthMinimalSerializer(many=True),
                500: {"description": "Unexpected error"}
            }
    )
    def get(self, request):
        try:
            ...
            return Response({'month_list': MonthMinimalSerializer(months, many=True).data}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'error': f'An unexpected error occurred: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
